...
def navigate(items: list[str], save_map: bool = True) -> dict:
    ...
    for step_num, target_node in enumerate(best_order, 1):
        ...
        try:
            ...
            # ── Sensor turn confirmation ───────────────────────────────────
            if _sensors.connected and direction in ("TL", "TR"):
                expected = "left" if direction == "TL" else "right"
                speak(f"Please turn {expected} now.", block=True)
                print(f"         [Sensors] Waiting for {expected} turn...")
                deadline  = _time.time() + 12
                confirmed = False
                while _time.time() < deadline:
                    if _sensors.detected_turn() == expected:
                        speak("Turn confirmed. Continue straight ahead.")
                        print(f"         [Sensors] Confirmed. Steps: {_sensors.step_count}  Dist: {_sensors.distance_m:.1f}m")
                        confirmed = True
                        break
                    _time.sleep(0.2)
                if not confirmed:
                    speak("If you have turned, continue ahead.")
            elif _sensors.connected:
                print(f"         [Sensors] Steps: {_sensors.step_count}  Dist: {_sensors.distance_m:.1f}m")
