...
# Shared sensor hub — connect your phone via "Sensor Server" app
# Update host to your phone's IP shown in the app
...
def draw_overlay(frame, ocr_results, navigator, detected, yolo_results=None):
    ...
    # ── Sensor HUD (bottom-left, above instruction banner) ────────────────
    if _sensors.connected:
        score    = _sensors.stability_score()
        steps    = _sensors.step_count
        dist     = _sensors.distance_m
        bar_col  = (0,255,0) if score > 0.7 else (0,165,255) if score > 0.4 else (0,0,255)
        bar_len  = int(score * 90)

        cv2.putText(frame, f"Steps:{steps}  Dist:{dist:.1f}m",
                    (10, h-110), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (200,200,200), 1)
        cv2.putText(frame, "Steady:", (10, h-92),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.40, (200,200,200), 1)
        cv2.rectangle(frame, (72, h-102), (162, h-90), (50,50,50), -1)
        cv2.rectangle(frame, (72, h-102), (72+bar_len, h-90), bar_col, -1)

        turn = _sensors.detected_turn()
        if turn != "none":
            speak(f"Turning {turn}.", block=False)
            cv2.putText(frame, f"TURN {turn.upper()}",
                        (w//2 - 60, 115), cv2.FONT_HERSHEY_SIMPLEX,
                        1.0, (0,255,255), 2)
...
def main():
    ...
    while True:
        ...
        if frame_count % 10 == 0:
            # ── Gyroscope stability gate — skip OCR on shaky frames ───────
            if _sensors.connected and not _sensors.is_stable():
                score = _sensors.stability_score()
                cv2.putText(frame, f"Hold steady  ({score:.0%})",
